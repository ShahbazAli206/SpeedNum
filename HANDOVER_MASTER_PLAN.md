...
12. [ ] **Admin console** (`/admin`, super-admin/platform view) — 100% demo (`getTenants()`/`getAudit()` fixtures), despite real backend support already existing: `GET /admin/tenants`, `GET /admin/stats` (backend/app/routers/admin.py, `SuperadminDep`-gated). No audit-log **read** endpoint exists yet for the audit-log panel — needs one added (model already exists: `AuditLog` in models.py, written to constantly via `audit.record(...)`, never read back). Reference pattern: `admin/backups/page.tsx` (already-real sibling page: `"use client"` + `useApi` + `EmptyState` on 403).
13. [ ] **Custom fields page** (`/custom-fields`) — 100% demo; real CRUD backend already exists (`GET/POST/PATCH/DELETE /custom-fields`, backend/app/routers/custom_fields.py). Small, mechanical wiring.
14. [ ] **Clients settings page** (`/clients/settings`) — 13 lines, not yet inspected this pass; check before assuming scope.
15. [ ] **Client detail page** (`/clients/[id]`) — biggest remaining gap. `page.tsx` still 100% demo (hardcoded `CLIENT_IDS` via `generateStaticParams`, real clients 404). `client-detail-client.tsx` (896 lines) additionally has three **fake local-only** flows that must not survive to handover: "Add service" (real `assignClientService` call exists but errors are silently swallowed with `.catch(() => {})`, and the row is added to permanent local state regardless of success), "Add task" (100% local state, never calls `POST /tasks` at all), "Add file" (uses `uploadDocument`/client-portal-scoped upload, not usable from the firm side for an arbitrary client). Real backend endpoints already exist for contacts (`/clients/{id}/contacts`, `/contacts/{id}`), services (`/clients/{id}/services` CRUD), tasks (`GET /tasks`, filter by `client_id` client-side same as workflows/[id]), custom fields (`GET /custom-fields`), letters (`GET /engagements`, check query support). **Missing**: a staff-side documents endpoint — `client_documents.py` is `/client-portal/documents`, scoped to a client-portal session (`ClientScopeDep`), not callable by staff on behalf of a client. `Document` model already has `client_id` (and `task_id`, added this project for task attachments) — the fix is a new staff-scoped router mirroring `task_attachments.py`'s exact pattern (`TenantUserDep` + `ensure_client_in_tenant`), e.g. `GET/POST /clients/{client_id}/documents`, `/upload-url`, `/{document_id}/download-url`, `DELETE /{document_id}`. No migration needed.
